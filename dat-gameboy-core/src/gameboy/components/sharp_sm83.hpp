...
namespace dat
{
	...
	public:
		...
		s_WordRegister DE;
		s_WordRegister HL;
		s_WordRegister SP;					/* Stack Pointer */
		s_WordRegister PC;					/* Program Counter */
	...
	private:
		...
		{
			u16 word = fetch_word();
			address_of(word) = SP.get_lsb();
			word++;
			address_of(word) = SP.get_msb();
		}

		inline void ldh_A_C()
		{
			A = address_of(0xFF00 + BC.get_lsb());
		}

		inline void ldh_C_A()
		{
			address_of(0xFF00 + BC.get_lsb()) = A;	
		}

		inline void ldh_addr_a8_A()
		{
			address_of(0xFF00 + fetch_byte()) = A;
		}

		inline void ldh_A_addr_a8()
		{
			A = address_of(0xFF00 + fetch_byte());
		}

		inline void pop_af()
		{
			u8 lsb = address_of(SP.get());
			++SP;

			u8 msb = address_of(SP.get());
			++SP;
	...
	private:
		// TODO Remove
		inline void nimp(const char* opcodeName) { DAT_LOG_WARN("Instruction not implemented: {}", opcodeName); }
