...
namespace dat
{
	...
	constexpr inline const char* get_cartridge_type(const s_CartridgeHeader& header)
	{
	...
	constexpr inline u32 get_rom_size_kib(const s_CartridgeHeader& header)
	{
		return 32 * (1 << header.romSize);
	}

	constexpr inline u32 get_ram_size_kib(const s_CartridgeHeader& header)
	{
		return 32 * (1 << header.ramSize);
	}
}
...
namespace dat
{
	...
	inline s_Cartridge read_cartrige(const std::filesystem::path& filepath)
	{
		...
		cartridge.buffer.size = static_cast<size_t>(fileSize - sizeof(s_CartridgeHeader));
		cartridge.buffer.data = (u8*)malloc(cartridge.buffer.size);
		cartridge.buffer.deleter = default_deleter;

		if (!cartridge.buffer.data)
		{
			DAT_LOG_CRITICAL("Failed to allocate cartridge buffer");
			return {};
		}

		file.seekg(0x100, std::ios::beg);
		file.read(reinterpret_cast<char*>(&cartridge.header), sizeof(s_CartridgeHeader));
		file.read(reinterpret_cast<char*>(cartridge.buffer.data), cartridge.buffer.size);
