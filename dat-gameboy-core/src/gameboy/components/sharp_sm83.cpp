...
namespace dat
{
	void s_SharpSM83::initialize(s_Memory* memory)
	{
		r_Memory = memory;
	}
	...
	u8& s_SharpSM83::address_of(u16 reg)
	{
		return (*r_Memory)[reg];
	}
...
namespace dat
{
	...
	void s_SharpSM83::dec(u8& target)
	{
		...
		F.Z = (target == 0);
		F.N = 1;
		F.H = check_half_borrow(originalTarget, 1);
	}
	...
	{
		--SP;
		address_of(SP.get()) = data.get_msb();
		--SP;
		address_of(SP.get()) = data.get_lsb();
	}

	void s_SharpSM83::push_byte(u8 data)
	{
		// TODO: check if PUSH a16, rr work
		--SP;
		address_of(SP.get()) = data;
	}
